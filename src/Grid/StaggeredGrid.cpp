...
inline float& StaggeredGrid::operator()(float x, float y)
{
  ...
  if (x_border)
  {
    if (y_border)
    {
      if (axis)
      {
        ...
        return (_data(xi, yi-1) + _data(xi, yi+1)) * 0.5;
      }
      if (xi == 0)
        return _data(0, yi);
      if (xi == _x_res)
        return _data(_x_res-1, yi);
      return (_data(xi-1, yi) + _data(xi, yi)) * 0.5;
    }
    return _data(xi, yi);
  }
  if (y_border)
  ...
  if (axis)
    return (_data(xi, yi-1) + _data(xi, yi+1)) * 0.5;
  return (_data(xi, yi) + _data(xi+1, yi)) * 0.5;
}
